...
- 트리·비교는 **`.repos` 매니페스트가 아니라 워크스페이스 각 repo 에 설정된 git
  remote(origin/upstream)** 를 참조한다. ahead/behind 는 각 repo 의 upstream 기준,
  Remote 컬럼은 `git remote get-url origin` 결과. (`vcs import` 만 `.repos --input` 사용)
...
| **Changes** | 변경된 파일 수 |
| **State** | `present` (워크스페이스에서 발견된 repo) |
| **Remote (origin)** | 각 repo 의 `origin` remote URL |

상태 색상: present=기본, dirty=주황.

repo 목록·상태는 `vcs custom --git --args status -sb`(중첩 repo 포함, 각 repo 의
upstream 기준 ahead/behind)와 `vcs custom --git --args remote get-url origin`(origin URL)
으로 수집한다. 별도의 `.repos` 매니페스트 비교는 하지 않는다.
...
| Remote/Branch 빈칸 | 해당 repo 에 `origin` remote 미설정, 또는 upstream 미추적 |
...
│   ├── repolist.py       # git status/remote 출력 → 트리 모델 (순수 함수)
│   └── settings.py       # 입력값 영속화
